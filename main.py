...
def apply_complex_filter(data: List[Dict], filter_str: str) -> List[Dict]:
    ...
    for item in data:
        ...
        conditions = re.findall(r"(\w+\s*(?:==|!=|>=|<=|>|<|regex)\s*[^()&| ]+)", filter_str)
...
def generate_openapi_spec(project: str):
    ...
    base = f"/api/v1/{project}"
    return {
        "openapi": "3.0.0",
        "info": {"title": f"API Docs: {project}", "version": "1.2.0"},
        "paths": {
            base: {
                "get": {
                    "tags": [project],
...
async def save_all(project: str, items: Any = Body(...)):
    save_project_data(project, items)
    return {"status": "ok"}

@app.get("/api/v1/{project}/{item_id}")
...
@app.put("/api/v1/{project}/{item_id}")
...
@app.delete("/api/v1/{project}/{item_id}")
